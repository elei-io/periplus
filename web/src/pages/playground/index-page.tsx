...
import { useIndexRun } from "@/hooks/use-index-run"
...
export function IndexPage() {
  ...
  const hasProgress = indexRun.events.length > 0
  const showResults = indexRun.isSuccess && !indexRun.isRunning
    ...
    >
      <section className="mx-auto grid w-full max-w-4xl gap-5">
        ...
        <IndexForm
          ...
          isLocked={false}
          isRunning={indexRun.isRunning}
          onCancel={indexRun.cancel}
          onSubmit={(input) => {
        ...
        >
          <IndexProgress events={indexRun.events} />
        </AnimatedSection>
      </section>
      ...
      >
        <IndexResults events={indexRun.events} links={indexRun.result} />
      </AnimatedSection>
    </div>
  )
}
