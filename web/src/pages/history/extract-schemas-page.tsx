...
import { useExtractSchemaMetrics, useExtractSchemas } from "@/hooks/use-history-data"
import { HistoryMetricsBand } from "@/pages/history/history-metrics"
import {
  HISTORY_PAGE_SIZE,
  HistoryPagination,
} from "@/pages/history/history-pagination"
import type {
  ExtractSchemaFilters,
  ExtractSchemaRecord,
} from "@/types/history"

const defaultFilters: ExtractSchemaFilters = {
  matchPattern: "",
  prompt: "",
  schemaType: "all",
  enabled: "all",
...
export function ExtractSchemasPage() {
  const [filters, setFilters] = useState<ExtractSchemaFilters>(defaultFilters)
  const [offset, setOffset] = useState(0)
  const schemasQuery = useExtractSchemas(filters, {
    limit: HISTORY_PAGE_SIZE,
    offset,
  })
  const metricsQuery = useExtractSchemaMetrics(filters)
  const schemas = schemasQuery.data?.items ?? []
  const total = schemasQuery.data?.total ?? 0

  const patchFilters = (patch: Partial<ExtractSchemaFilters>) => {
    setOffset(0)
  ...
  return (
    <div className="flex min-h-0 w-full flex-col gap-4">
      <section className="flex flex-col gap-3 border-b pb-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h1 className="truncate text-lg font-medium">Extract Schemas</h1>
        ...
        <div className="grid gap-2 xl:grid-cols-[minmax(16rem,1fr)_minmax(14rem,1fr)_9rem_9rem_10rem]">
          ...
          <FilterSelect
            ...
            onChange={(schemaType) =>
              patchFilters({ schemaType: schemaType as ExtractSchemaFilters["schemaType"] })
            }
          ...
          <FilterSelect
            ...
            onChange={(enabled) =>
              patchFilters({ enabled: enabled as ExtractSchemaFilters["enabled"] })
            }
          ...
          <FilterSelect
            ...
            onChange={(warnings) =>
              patchFilters({ warnings: warnings as ExtractSchemaFilters["warnings"] })
            }
      ...
      <Table containerClassName="min-h-0 flex-1 rounded-md border bg-card/80">
        ...
        <TableBody>
          {schemas.map((schema) => (
            <ExtractSchemaRow key={schema.id} schema={schema} />
          ))}
          {!schemasQuery.isLoading && schemas.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} className="h-24 text-center text-muted-foreground">
                No extract schemas match.
              </TableCell>
            </TableRow>
          ) : null}
        </TableBody>
...
function ExtractSchemaRow({ schema }: { schema: ExtractSchemaRecord }) {
  return (
    <TableRow>
      <TableCell className="max-w-[24rem]">
        <a
          href={`/history/extract-schemas/${schema.id}`}
