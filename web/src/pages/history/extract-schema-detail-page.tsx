...
import { useExtractSchema, useUpdateExtractSchema } from "@/hooks/use-history-data"
import type { ExtractSchemaDetailRecord } from "@/types/history"

export function ExtractSchemaDetailPage({ schemaId }: { schemaId: string }) {
  const schemaQuery = useExtractSchema(schemaId)
  const schema = schemaQuery.data

  if (schemaQuery.isLoading) {
    return <CenteredText>Loading extract schema...</CenteredText>
  }
    ...
    <div className="flex min-h-0 w-full flex-col gap-4 overflow-y-auto">
      <section className="flex flex-wrap items-center justify-between gap-3 border-b pb-4">
        <div className="flex min-w-0 items-center gap-2">
          ...
          <h1 className="truncate text-lg font-medium">Extract Schema</h1>
...
function SchemaAdminEditor({ schema }: { schema: ExtractSchemaDetailRecord }) {
  const updateSchema = useUpdateExtractSchema(schema.id)
...
function LinkActions({ schema }: { schema: ExtractSchemaDetailRecord }) {
  return (
...
function BackButton() {
  return (
    <Button
      ...
      size="sm"
      nativeButton={false}
      render={<a href="/history/extract-schemas" />}
    >
      <ArrowLeftIcon />
      Extract Schemas
    </Button>
...
function WarningDetails({ schema }: { schema: ExtractSchemaDetailRecord }) {
