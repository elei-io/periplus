...
import { useCrawlPolicy, useDeleteCrawlPolicy, useUpdateCrawlPolicy } from "@/hooks/use-resource-data"
...
export function CrawlPolicyDetailPage({ policyId }: { policyId: string }) {
  ...
  return <div className="flex min-h-0 w-full flex-col gap-4">
    <section className="flex flex-wrap items-start justify-between gap-3 border-b pb-4"><div className="flex min-w-0 items-start gap-3"><Button variant="ghost" size="icon-sm" nativeButton={false} render={<a href="/crawl-policies" />}><ArrowLeftIcon /></Button><div className="min-w-0"><div className="flex items-center gap-2"><ShieldCheckIcon className="size-4 text-muted-foreground" /><h1 className="truncate text-lg font-medium">{coverageLabel(policy)}</h1></div><div className="mt-1"><Badge variant={policy.enabled ? "secondary" : "destructive"}>{policy.enabled ? <CheckCircle2Icon /> : <XCircleIcon />}{policy.enabled ? "Enabled" : "Disabled"}</Badge></div></div></div><Button variant="outline" size="sm" disabled={query.isFetching} onClick={() => void query.refetch()}><RefreshCwIcon className={query.isFetching ? "animate-spin" : ""} /> Refresh</Button></section>
    <ExistingPolicyForm key={policy.updated_at} policy={policy} />
...
function ExistingPolicyForm({ policy }: { policy: CrawlPolicyDetailRecord }) {
  ...
  return <><PolicyForm draft={draft} onChange={setDraft} defaultPolicy={isDefault} /><div className={`sticky bottom-3 z-10 flex items-center gap-3 ${isDefault ? "justify-end" : "justify-between"}`}>{!isDefault ? <Button variant="destructive" disabled={remove.isPending} onClick={deletePolicy}><Trash2Icon /> Delete policy</Button> : null}<Button className="shadow-lg" disabled={update.isPending} onClick={save}><SaveIcon /> {update.isPending ? "Saving…" : "Save policy"}</Button></div></>
}

function coverageLabel(policy: CrawlPolicyDetailRecord) { const host = policy.host.startsWith("*.") ? `subdomains of ${policy.host.slice(2)}` : policy.host; if (policy.host === "*") return "All websites and pages"; if (policy.path_prefix === "/" && policy.path_mode === "prefix") return `All pages on ${host}`; return policy.path_mode === "exact" ? `${host}${policy.path_prefix} only` : `${host}${policy.path_prefix} and below` }
