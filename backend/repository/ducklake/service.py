...
class CatalogueService:
    ...
    def crawl_health_since(self, captured_after: datetime) -> dict[str, Any]:
        """Return bounded aggregate crawl health from the durable owner."""

        table = self._table("crawls")
        success = (
            "(status_code IS NULL OR status_code BETWEEN 200 AND 399) "
            "AND json_array_length(errors_json) = 0"
        )
        summary = self.catalogue.lake.sql_dicts(
            "SELECT count(*) AS total, "
            f"count(*) FILTER (WHERE {success}) AS succeeded, "
            "coalesce(quantile_cont(duration_ms / 1000.0, 0.5) "
            "FILTER (WHERE duration_ms IS NOT NULL), 0) AS duration_p50_seconds, "
            "coalesce(quantile_cont(duration_ms / 1000.0, 0.95) "
            "FILTER (WHERE duration_ms IS NOT NULL), 0) AS duration_p95_seconds "
            f"FROM {table} WHERE captured_at >= $captured_after",
            captured_after=captured_after,
        )[0]
        domains = self.catalogue.lake.sql_dicts(
            "SELECT regexp_extract(normalized_url, '^https?://([^/:?#]+)', 1) "
            "AS domain, count(*) AS total, "
            f"count(*) FILTER (WHERE NOT ({success})) AS failed, "
            "coalesce(quantile_cont(duration_ms / 1000.0, 0.95) "
            "FILTER (WHERE duration_ms IS NOT NULL), 0) AS duration_p95_seconds "
            f"FROM {table} WHERE captured_at >= $captured_after "
        ...
        failures = self.catalogue.lake.sql_dicts(
            "SELECT CASE "
            "WHEN status_code = 429 THEN 'http_429' "
            "WHEN status_code BETWEEN 400 AND 499 THEN 'http_4xx' "
            "WHEN status_code BETWEEN 500 AND 599 THEN 'http_5xx' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%dns%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%name resolution%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%nodename nor servname%' "
            "THEN 'dns' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%connect timeout%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%connection timed out%' "
            "THEN 'connect_timeout' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%read timeout%' "
            "THEN 'read_timeout' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%ssl%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%tls%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%certificate%' THEN 'tls' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%browser%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%target closed%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%page crashed%' "
            "THEN 'browser_crash' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%blocked%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%captcha%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%access denied%' "
            "THEN 'content_blocked' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%capacity%' "
            "OR lower(cast(errors_json AS VARCHAR)) LIKE '%permit%' "
            "THEN 'capacity_timeout' "
            "WHEN lower(cast(errors_json AS VARCHAR)) LIKE '%lease%' "
            "THEN 'capacity_lease_lost' "
            "ELSE 'navigation' END AS reason, count(*) AS count "
            f"FROM {table} WHERE captured_at >= $captured_after AND NOT ({success}) "
