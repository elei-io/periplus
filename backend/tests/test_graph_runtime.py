...
class GraphRuntimeTests(unittest.TestCase):
    def test_historical_edges_pin_the_catalogue_snapshot_at_run_creation(self) -> None:
        async def scenario() -> None:
            ...
            run = await create_graph_run(
                ...
                catalogue_snapshot_resolver=latest_snapshot,
            )

            self.assertEqual(run.catalogue_snapshot_id, 42)
    ...
    def test_successful_acquisition_publishes_navigation_readiness(self) -> None:
        async def scenario() -> None:
            runs, requests = FakeKV(), FakeKV()
            graph = snapshot()
            run = new_graph_run(graph, ["https://example.com"])
            ...
            with (
                patch("workers.acquisition.crawl_graph_request", acquire),
                patch(
                    "workers.acquisition._derive_navigation_package",
            ...
            self.assertTrue(acquire.await_args.kwargs["include_html"])
            self.assertEqual(len(jetstream.messages), 1)
            subject, payload = jetstream.messages[0]
            self.assertEqual(subject, NAVIGATION_READINESS_SUBJECT)
            readiness = NavigationReadinessWork.model_validate_json(payload)
            self.assertEqual(readiness.navigation, package)
            current = await get_crawl_request(requests, request.id)
            assert current is not None
            self.assertEqual(current.status, "awaiting_navigation")
            Message.ack.assert_awaited_once()

        asyncio.run(scenario())

    def test_readiness_publish_failure_releases_the_crawl_for_redelivery(self) -> None:
        async def scenario() -> None:
            runs, requests = FakeKV(), FakeKV()
            graph = snapshot()
            run = new_graph_run(graph, ["https://example.com"])
            ...
            with (
                ...
                patch(
                    "workers.acquisition._derive_navigation_package",
                    AsyncMock(return_value=navigation_package()),
                ),
                patch("workers.acquisition.transition_node_progress", AsyncMock()),
                patch("workers.acquisition.settle_request", AsyncMock()) as settle,
            ):
            ...
            self.assertEqual(current.document_id, "sha256:" + "a" * 64)
    ...
    def test_acquisition_infrastructure_failure_releases_claim_for_redelivery(
        self,
    ) -> None:
        async def scenario() -> None:
            runs, requests = FakeKV(), FakeKV()
            ...
            with (
                patch(
                    ...
                    AsyncMock(side_effect=OSError("NATS unavailable")),
                ),
                patch("workers.acquisition.transition_node_progress", AsyncMock()),
                patch("workers.acquisition.settle_request", AsyncMock()) as settle,
            ):
    ...
    def test_contracts_and_identities(self) -> None:
        value = snapshot()
        run = new_graph_run(
            value, ["https://EXAMPLE.com", "https://example.org/a#fragment"]
        )
        self.assertEqual(
            run.trigger_urls, ("https://example.com/", "https://example.org/a")
        )
        with self.assertRaisesRegex(ValueError, "root node"):
            new_graph_run(snapshot(entry=False), ["https://example.com"])
        ...
        self.assertEqual(
            request_identity(run_id, "https://example.com/a#one"),
            request_identity(run_id, "https://EXAMPLE.com/a#two"),
        )
        self.assertNotEqual(
            request_identity(
                run_id,
        ...
        self.assertEqual(
            request_identity(
                ...
                source_edge_id=edge_id,
                source_document_id="sha256:same",
            ),
            request_identity(
                run_id,
                "https://example.com/a",
        ...
        identity = request_identity(uuid4(), "https://example.com")
        self.assertEqual(
            deterministic_request_id(identity), deterministic_request_id(identity)
        )
        ...
        payload = NavigationReadinessWork(
            event_id=uuid4(),
            crawl_id=uuid4(),
            graph_run_id=uuid4(),
    ...
    def test_admission_is_idempotent_and_freezes_policy(self) -> None:
        async def scenario() -> None:
            ...
            def resolver(_url: str):
                return policy_snapshot("browser")
    ...
    def test_navigation_readiness_activates_edges_without_materialization_state(
        self,
    ) -> None:
        async def scenario() -> None:
            runs, requests, progress, jetstream = (
    ...
    def test_navigation_readiness_clears_an_acquisition_claim(self) -> None:
        async def scenario() -> None:
            runs, requests, progress, jetstream = (
                FakeKV(),
                FakeKV(),
            ...
            self.assertEqual(current.status, "evaluating_edges")
            self.assertIsNone(current.claim_token)
            self.assertIsNone(current.claim_expires_at)
    ...
    def test_cancellation_settles_every_nonterminal_request(self) -> None:
        async def scenario() -> None:
            ...
            for url, status in (
                ...
                await requests.create(
                    request.id.hex, request.model_dump_json().encode()
                )
    ...
    def test_edge_redelivery_does_not_duplicate_target_request(self) -> None:
        async def scenario() -> None:
            ...
            await runs.create(run.id.hex, run.model_dump_json().encode())
            await initialize_run_progress(progress, run)
            source, _admitted = await admit_request(
                runs=runs,
                requests=requests,
                progress=progress,
                jetstream=jetstream,
        ...
        async def scenario() -> None:
            ...
            with patch(
                ...
                side_effect=lambda name: (
                    0.01 if name == "ATLAS_EDGE_QUERY_TIMEOUT_SECONDS" else 60
                ),
            ):
                with self.assertRaisesRegex(
                    EdgeEvaluationFailed, "execution-time limit"
                ):
                    await evaluate_edge(
                        runs=runs,
                        requests=requests,
        ...
        async def run_mode(mode: EdgeDedupeMode) -> tuple[int, int]:
            ...
            for path in ("one", "two"):
                source, admitted = await admit_request(
                    ...
                    run_id=run.id,
                    node_id=graph.nodes[0].id,
                    url=f"https://source.example/{path}",
                    policy_resolver=lambda _url: policy_snapshot(),
                )
                assert source is not None and admitted
                source = await update_crawl_request(
                    ...
                    lambda value: value.model_copy(
                        update={"document_id": "sha256:same-document"}
                    ),
                )
