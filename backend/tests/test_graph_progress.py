...
class GraphProgressTests(unittest.TestCase):
    def test_component_progress_is_derived_from_current_request_state(self) -> None:
        async def scenario() -> None:
            ...
            source = CrawlRequest(id=uuid4(), graph_run_id=run_id, node_id=source_id, url="https://example.com/", transport="http", status="evaluating_edges", created_at=now, updated_at=now)
            target = CrawlRequest(id=uuid4(), graph_run_id=run_id, node_id=target_id, url="https://target.example/", transport="http", source_edge_id=edge_id, status="crawling", created_at=now, updated_at=now)
