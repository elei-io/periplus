...
| Runtime document scope for managed `elements` scans | Implemented | Implemented | Direct finite `document_id` predicates, finite disjunctions, and statically empty scans remain single-stage. Direct unfiltered row counts bypass DOM staging because DuckLake folds them from exact snapshot metadata. Direct unordered `LIMIT`/`FETCH` scans are admitted within the interactive row budget. Selective managed relationship lineage through inner equijoins, `IN`, correlated `EXISTS`, CTEs, and literal `VALUES` produces a typed staged plan; separate literal lookups budget document metadata and hydrate only referenced element columns into one shared temporary relation on one transaction-pinned Quack session. Other unprovable scans fail closed with `unbounded_relation`; plain `EXPLAIN` does not execute, while `EXPLAIN ANALYZE` receives the normal proof. |
