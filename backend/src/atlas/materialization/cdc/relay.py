"""Ingress Basin-owned DuckLake CDC into Atlas-owned CDC subjects."""
...
from atlas.materialization.cdc import metrics
...
async def run(
    *,
    ...
    basin_client = await connect_basin_cdc()
    atlas_client = await connect_nats()
    basin_jetstream = basin_client.jetstream()
    atlas_jetstream = atlas_client.jetstream()
    catalogue: Catalogue | None = None
    catalogue_executor = ThreadPoolExecutor(
        max_workers=1, thread_name_prefix="cdc"
    )
    pulls: dict[str, asyncio.Task] = {}
    try:
        await ensure_cdc_stream(atlas_jetstream)
        loop = asyncio.get_running_loop()
        catalogue = await loop.run_in_executor(
            catalogue_executor, catalogue_from_env
        )
        ...
        await loop.run_in_executor(catalogue_executor, resolver.resolve, set())
            ...
            if dml_task.done():
                for message in dml_task.result():
                    await _publish_dml_message(
                        ...
                        resolver,
                        message,
                        catalogue_executor=catalogue_executor,
                    )
        ...
        metrics.failure(phase="basin_ingress")
        if monitor is not None:
        ...
        if catalogue is not None:
            await asyncio.get_running_loop().run_in_executor(
                catalogue_executor, catalogue.close
            )
        catalogue_executor.shutdown(wait=True, cancel_futures=True)
        await asyncio.gather(
            basin_client.close(),
            atlas_client.close(),
            return_exceptions=True,
...
async def _publish_dml_message(
    ...
    message,
    *,
    catalogue_executor: ThreadPoolExecutor | None = None,
) -> None:
    source = BasinDMLTick.model_validate_json(message.data)
    table_ids = set(source.table_ids)
    async def resolve_tables():
        if catalogue_executor is None:
            return resolver.resolve(table_ids)
        return await asyncio.get_running_loop().run_in_executor(
            catalogue_executor, resolver.resolve, table_ids
        )
