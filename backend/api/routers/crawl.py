...
from actions.crawl.schemas import Input, CrawlOutput
...
async def _crawl_stream(session: Session, request: Input) -> AsyncIterator[str]:
    queue: asyncio.Queue[CrawlProgressEvent | CrawlOutput | Exception | object] = asyncio.Queue()
    ...
    try:
        while True:
            ...
            if isinstance(item, CrawlOutput):
...
@router.post("/", response_model=CrawlOutput)
async def crawl(
    request: Input,
    http_request: Request,
    session: Annotated[Session, Depends(get_session)],
) -> CrawlOutput | StreamingResponse:
    if "text/event-stream" in http_request.headers.get("accept", ""):
        return StreamingResponse(
            _crawl_stream(session=session, request=request),
