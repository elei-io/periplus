...
const projections: ProjectionDefinition[] = [
  ...
  },
  {
    ...
    description: "Flattened structured-data values embedded in documents.",
    source: "ingest.documents",
  },
  {
    value: "links",
  ...
  },
  {
    ...
    description: "Visit-level status, timing, and document evidence.",
    source: "ingest.visits",
  },
]

const projectionValues = projections.map((projection) => projection.value)
const htmlDependents = new Set<Projection>([
  "jsonld_values",
  "links",
  "link_observations",
])
const linkPair = new Set<Projection>(["links", "link_observations"])

export function MaterializationsPage() {
  const client = useQueryClient()
  const [mode, setMode] = useState<"backfill" | "rebuild">("rebuild")
  const [selected, setSelected] = useState<Projection[]>(["pages"])
  const [expanded, setExpanded] = useState<Projection[]>([])
  const runs = useQuery({
  ...
  const included = useMemo(() => {
    ...
    if (values.has("html_elements")) {
      values.add("jsonld_values")
      values.add("links")
      values.add("link_observations")
    }
    if ([...values].some((value) => linkPair.has(value))) {
      values.add("links")
      values.add("link_observations")
    }
    return values
  ...
  const toggle = (projection: Projection) => {
    setSelected((current) =>
  ...
  const toggleExpanded = (projection: Projection) => {
    setExpanded((current) =>
      current.includes(projection)
        ? current.filter((item) => item !== projection)
        : [...current, projection]
  ...
  return (
    <div className="flex min-h-0 w-full flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <p className="max-w-xl text-xs text-muted-foreground">
              {mode === "rebuild"
                ...
                : "Repairs selected live tables in bounded, resumable batches."}
            </p>
          </div>
          <Button
            size="sm"
        ...
        <Table>
          ...
          <TableBody>
            {projections.map((projection) => {
              ...
              const inherited =
                ((selected.includes("html_elements") &&
                  htmlDependents.has(projection.value)) ||
                  (selected.some((value) => linkPair.has(value)) &&
                    linkPair.has(projection.value))) &&
                !selected.includes(projection.value)
                  ...
                  >
                    ...
                    <TableCell className="py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {inherited && (
                          <Badge variant="outline">included dependency</Badge>
                        )}
                      </div>
      ...
      <p className="px-1 text-[0.6875rem] text-muted-foreground">
        Selecting <code>material.html_elements</code> also includes JSON-LD
        values and links so their shared document projection remains
        consistent.
      </p>
    </div>
  )
