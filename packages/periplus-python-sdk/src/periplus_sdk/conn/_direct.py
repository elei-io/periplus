"""Direct read-only Periplus DuckLake connection."""
...
class DuckConfig:
    ...
    def from_values(cls, values: Mapping[str, str | None]) -> Self:
        alias = _required(values, "PERIPLUS_DUCKLAKE_ALIAS")
        if not _ALIAS.fullmatch(alias):
            raise ConfigurationError(
                "PERIPLUS_DUCKLAKE_ALIAS must be a SQL identifier"
            )
        data_path = _required(values, "PERIPLUS_DUCKLAKE_DATA_PATH")
        return cls(
            alias=alias,
...
def duck(
    ...
    *,
    ...
    if config is None:
        env_file = os.getenv("PERIPLUS_DIRECT_ENV_FILE")
        config = (
...
def _s3_config(
    ...
    if urlsplit(data_path).scheme.lower() != "s3":
        return None
    key_id = _first(values, "PERIPLUS_DUCKLAKE_S3_KEY_ID", "AWS_ACCESS_KEY_ID")
    ...
    url_style = _value(values, "PERIPLUS_DUCKLAKE_S3_URL_STYLE")
    ...
    return S3Config(
        endpoint=_value(values, "PERIPLUS_DUCKLAKE_S3_ENDPOINT"),
        region=(
            _value(values, "PERIPLUS_DUCKLAKE_S3_REGION")
        ...
        url_style=url_style,
        use_ssl=_optional_bool(values, "PERIPLUS_DUCKLAKE_S3_USE_SSL"),
    )
