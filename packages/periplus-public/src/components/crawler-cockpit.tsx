...
function CaptureCount({ motion }: { motion: boolean }) {
  ...
  return <div className="crawler-total"><span className="crawler-metric-label">Captures recorded</span><strong>{query.error ? "—" : value !== null ? String(value).replace(/\B(?=(\d{3})+(?!\d))/g, ",") : "…"}</strong><span>{query.error ? "Capture total unavailable" : "Successful captures · includes repeat visits"}</span></div>
}
...
export function CrawlerCockpit({ initialId, initialDescription }: { initialId?: string; initialDescription?: string }) {
  ...
  const recent: CockpitItem[] = captures.visible.map(item => ({ id: item.observation_id, url: item.requested_url, stage: "past", label: `Captured · ${age(item.completed_at, captures.query.data?.as_of ?? current?.as_of ?? item.completed_at)}`, origin: item.query_ready ? "Ready to query" : "Query readiness not yet verified" }))
  useLayoutEffect(() => {
  ...
  const status = live.isError ? "Connection interrupted" : live.isPending ? "Connecting…" : !motion ? "Updates paused" : current?.paused ? "Captures paused" : current?.started ? "Capturing the web" : current?.dispatched ? "Preparing captures" : current?.queued ? "Work waiting" : "Awaiting new pages"
  const closeRequest = () => {
    setRequestId(null)
  ...
  return <main className={`crawler-cockpit ${!motion ? "crawler-updates-paused" : ""}`}>
    ...
    {captures.query.data?.reset_reason && <p className="crawler-footnote" role="status">The live feed resynced after an interruption. Showing the latest captures; earlier results remain in the catalogue.</p>}
    <div className="crawler-workspace" aria-label="Coverage activity">
      <section className="crawler-stream-panel" aria-labelledby="capture-stream-heading">
        <div className="crawler-instruments"><Speedometer rate={motion && !reading && !captures.query.isError ? captures.query.data?.rate : undefined} /><CaptureCount motion={motion} /></div>
        <header className="crawler-stream-heading"><div><h2 id="capture-stream-heading">Latest captures</h2></div><span className="crawler-reading-status" role="status">{reading && motion ? "Held while you read" : ""}</span></header>
        <div ref={stream} className="crawler-stream-table" onMouseEnter={() => { if (captures.visible.length) setReading(true) }} onMouseLeave={event => { if (!event.currentTarget.contains(document.activeElement)) setReading(false) }} onFocusCapture={() => setReading(true)} onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget) && !event.currentTarget.matches(":hover")) setReading(false) }}><Table aria-label="Latest captures"><TableHeader><TableRow><TableHead>Page</TableHead><TableHead>Captured</TableHead><TableHead><span className="sr-only">Open website</span></TableHead></TableRow></TableHeader><TableBody>{recent.map(item => <TableRow key={item.id} data-capture-id={item.id} className="crawler-stream-row"><TableCell><a href={item.url} target="_blank" rel="noopener noreferrer" className="crawler-row-url" title={`${item.url} (opens in a new tab)`}><Check size={13} /><span>{item.url.replace(/^https?:\/\//, "")}</span></a></TableCell><TableCell>{item.label.replace(/^Captured · /, "")}</TableCell><TableCell><a className={buttonVariants({ variant: "ghost", size: "icon-sm" })} href={item.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${item.url} in a new tab`}><ArrowUpRight size={14} /></a></TableCell></TableRow>)}</TableBody></Table>{!recent.length && <div className="crawler-empty">{unavailable ? "Waiting for a live snapshot." : "New captures will appear here as pages are explored."}</div>}</div>
        <footer className="crawler-stream-footer"><span>{current ? `Snapshot ${new Date(current.as_of).toLocaleTimeString()} · updates every 5 seconds` : "Loading coverage activity"}</span><span>{captures.burst > 1 ? `${captures.burst} captures in the latest burst · ` : ""}Links open the live website in a new tab <TooltipProvider><Tooltip><TooltipTrigger aria-label="About capture metrics" className="crawler-metrics-info"><Info size={13} /></TooltipTrigger><TooltipContent>Totals and rate count successful captures, including repeat visits. Recorded totals may follow a little behind the live feed. The dial adjusts its labeled scale to the current rate; it is not a capacity limit.</TooltipContent></Tooltip></TooltipProvider></span></footer>
        
      </section>
      <aside className="crawler-frontier-panel" aria-labelledby="frontier-heading"><header><span className="crawler-lane-eyebrow">QUEUED WEBSITES <ArrowRight size={14} /></span><h2 id="frontier-heading">Waiting to be collected</h2><p>Websites with pages queued for capture.</p></header>
    ...
    <Dialog open={requestId !== null} onOpenChange={open => { if (!open) closeRequest() }}>
      <DialogContent className="observatory-request-dialog">
        <DialogHeader className="sr-only"><DialogTitle>Request progress</DialogTitle><DialogDescription>Capture results and scope for this request.</DialogDescription></DialogHeader>
