...
export function PublicNav() {
  ...
  return <header className="public-header">
    <div className="public-header-inner">
      ...
      <nav className="workspace-nav flex-wrap max-[380px]:grid! max-[380px]:grid-cols-3" aria-label="Main navigation">{destinations.map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={pathname === href || pathname.startsWith(`${href}/`) ? "page" : undefined}><Icon aria-hidden="true" />{label}</Link>)}</nav>
