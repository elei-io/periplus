...
function RequestSummary({ request }: { request: CoverageRequest }) {
  ...
  return <div className="flex flex-col gap-3">
    ...
    {progress && <CardDescription>{progress.acquisition_settled_count.toLocaleString()} page requests past acquisition · {progress.acquisition_pending_count.toLocaleString()} queued or fetching · {progress.navigation_pending_count.toLocaleString()} processing links · {progress.failed_request_count.toLocaleString()} failed{progress.crawl_limit_reached ? " · Page budget reached" : ""}{progress.status === "paused" ? " · Paused" : ""}</CardDescription>}
    {request.allowed_sections.length > 0 && <details><summary>Allowed sections ({request.allowed_sections.length})</summary><ul>{request.allowed_sections.map(section => <li key={section} className="break-all">{section}</li>)}</ul></details>}
...
export function CoverageRequestDetail({ id }: { id: string }) {
  ...
  return <Card id="request"><CardHeader><CardTitle><h2>Request details</h2></CardTitle><CardDescription>Keep this page’s URL to return to this request.</CardDescription></CardHeader><CardContent className="flex flex-col gap-4">
    ...
    {query.data && <><RequestSummary request={query.data} /><CardDescription>{query.data.status === "pending" ? "Waiting for automatic processing. No approval is needed." : query.data.status === "resolving" ? "Finding relevant public starting pages using search." : query.data.status === "ongoing" ? "Acquisition totals include failed or cancelled page requests. Pages can be available to query while their outgoing links are still processing. Counts update automatically." : query.data.status === "failed" ? "This request needs attention. See the explanation above." : "Collection has finished. Some pages may have failed, and data may still be processing before it is queryable."}</CardDescription>{query.data.resolved_urls.length > 0 && <details><summary>Starting pages ({query.data.resolved_urls.length})</summary><ul className="flex flex-col gap-2 py-3">{query.data.resolved_urls.map(url => <li key={url} className="break-all"><a className="story-link" href={url} target="_blank" rel="noopener noreferrer">{url}</a></li>)}</ul></details>}
