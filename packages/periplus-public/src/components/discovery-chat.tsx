...
const starters = [
  { label: "Compare book prices", text: "Compare titles and prices from collected Books to Scrape product pages. Pair the main heading with its sibling price and retain source URLs and collection dates.", detail: "A price comparison on practice data", icon: Globe2 },
  { label: "Explore page topics", text: "Show headings with source URLs and collection dates from a small sample of distinct collected pages.", detail: "Build a source-linked heading index", icon: Code2 },
...
export function DiscoveryChat({ initialPrompt, autoRun = false, onOpenSql }: { initialPrompt?: string; autoRun?: boolean; onOpenSql: (sql: string) => void }) {
  ...
  return <section className={`discovery-shell agent-workspace ${messages.length ? "has-conversation" : ""}`} aria-label="Ask workspace">
    <div className="agent-toolbar"><h2>{messages.length ? "Conversation" : "Explore the web as data"}</h2>{messages.length > 0 && <Button variant="outline" size="sm" disabled={busy} onClick={() => setMessages([])}><Plus />New chat</Button>}</div>
    ...
    <div className="composer-dock">
      <form className="discovery-composer" onSubmit={event => { event.preventDefault(); send(prompt) }}>
        <div className="composer-caption"><span><Search />Explore the web as data</span><span>Natural language → SQL → data</span></div>
        <Textarea aria-label="Ask Periplus" placeholder={messages.length ? "How would you like to refine the analysis?" : "What would you like to analyse?"} value={prompt} maxLength={4000} onChange={event => setPrompt(event.target.value)} className="composer-input" rows={2} onKeyDown={event => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); send(prompt) } }} />
    ...
    {!messages.length && <>
      <p className="story-note">Ask in ordinary language, or start with a <Link className="story-link" href="/datasets">named dataset</Link>. <Link className="story-link" href="/coverage">See available sites and dates</Link> before choosing a question. Questions explore the existing corpus; they do not add new pages.</p>
