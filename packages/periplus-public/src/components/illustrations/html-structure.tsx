    ...
    <div className={styles.frame}>
      <div className={styles.split}>
        <div className={styles.sourcePanel}><div className={styles.bar}>coast.html <span>Illustrative source</span></div><div className={styles.source} aria-label="HTML source">{sourceLines.map((line, index) => {
          ...
          return <button key={index} type="button" className={`${styles.sourceLine} ${selected && linesByTag[selected.tag] === index ? styles.selected : ""}`} disabled={!tag} aria-label={tag ? `Inspect ${tag} source` : `Source line ${index + 1}`} aria-pressed={Boolean(selected && linesByTag[selected.tag] === index)} onClick={() => tag && select(tag)}><span className={styles.lineNumber}>{index + 1}</span><code><SourceLine text={line} /></code></button>
        })}</div></div>
