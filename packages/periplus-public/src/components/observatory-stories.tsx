...
export function RequestStory({id, playing}: {id:string;playing:boolean}) {
  ...
  return <section className="observatory-request-story">
    ...
    <div className="request-summary-details">
      ...
      <dl className="request-summary-scope"><div><dt>Reach</dt><dd>{depth === 0 ? "Starting pages only" : `Up to ${depth} link ${depth === 1 ? "step" : "steps"}`}</dd></div><div><dt>Links per page</dt><dd>{item.specification.follow_link_limit.toLocaleString()}</dd></div><div><dt>Page budget</dt><dd>{item.specification.page_limit.toLocaleString()} pages</dd></div><div><dt>Retention</dt><dd>{item.retention_expired ? "Expired · results may be removed" : item.expires_at ? `Until ${new Date(item.expires_at).toLocaleDateString()}` : item.specification.retention_seconds === null ? "No scheduled expiry" : `${Math.ceil(item.specification.retention_seconds / 86400)} days after completion`}</dd></div></dl>
