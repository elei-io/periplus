...
function progress(item: Collection) {
  ...
  if (item.source === "history" || item.status === "settled") return "This request has finished. Its recorded captures remain available to explore."
  if (item.status === "paused") return "This request is paused. Captures already recorded remain available."
...
function RequestObservationTail({id, playing}: {id:string;playing:boolean}) {
  ...
  return <section ref={tail} className="request-observation-tail" aria-label="Latest captures for this request">
    <header><h4>Latest captures</h4><span>{query.isError ? "Updates interrupted" : playing ? "Updates every 5s" : "Updates paused"}</span></header>
...
export function RequestStory({id, playing}: {id:string;playing:boolean}) {
  ...
  const schedule=item.source === "current" ? item.schedule : null
  return <section className="observatory-request-story">
    ...
    <div className="request-summary-result"><div className="request-summary-count"><strong>{item.supplied_pages?.toLocaleString() ?? "—"}</strong><span>{item.supplied_pages === 1 ? "page captured" : "pages captured"}</span></div><p>{finished ? item.supplied_pages === 0 ? "This request finished without any successful captures." : item.supplied_pages === null ? "This request has finished. Explore any captures recorded so far." : "Captures are kept according to this request’s retention period." : progress(item)}</p></div>
    <div className="request-summary-details">
      ...
      {Boolean(item.failed_pages) && <p>{item.failed_pages?.toLocaleString()} {item.failed_pages === 1 ? "page could" : "pages could"} not be captured.</p>}
      <dl className="request-summary-scope"><div><dt>Reach</dt><dd>{depth === 0 ? "Starting pages only" : `Up to ${depth} link ${depth === 1 ? "step" : "steps"}`}</dd></div><div><dt>Links per page</dt><dd>{item.specification.follow_link_limit.toLocaleString()}</dd></div><div><dt>Pages per collection</dt><dd>{item.specification.page_limit.toLocaleString()} pages</dd></div><div><dt>Keep results</dt><dd>{item.retention_expired ? "Expired · results may be removed" : item.expires_at ? `Until ${new Date(item.expires_at).toLocaleDateString()}` : item.specification.retention_seconds === null ? "No scheduled expiry" : `${Math.ceil(item.specification.retention_seconds / 86400)} days after completion`}</dd></div></dl>
      {schedule && <p>{!schedule.enabled ? "Repeat collections are paused." : schedule.next_at ? `Checks for updates ${schedule.interval_seconds === 86400 ? "every day" : schedule.interval_seconds === 604800 ? "every week" : schedule.interval_seconds === 2592000 ? "every 30 days" : "on a schedule"}. Next planned check: ${new Date(schedule.next_at).toLocaleString()}.` : "No more repeat collections are planned."} {schedule.enabled && schedule.next_at && "Timing depends on site limits and availability."} {schedule.last_request_id && schedule.last_request_id !== item.id && <a href={`/coverage?request=${schedule.last_request_id}#request`}>View the latest collection</a>}</p>}
