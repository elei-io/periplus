...
export function LandingLauncher() {
  ...
  return <div className="landing-launcher">
    <form className="discovery-composer" onSubmit={event => { event.preventDefault(); launch() }}>
      <div className="launcher-toolbar"><Tabs value={mode} onValueChange={value => setMode(String(value))}><TabsList aria-label="Input mode"><TabsTrigger value="agent"><MessageSquare />Ask</TabsTrigger><TabsTrigger value="sql"><Code2 />SQL</TabsTrigger></TabsList></Tabs><span>Free to explore · No account needed</span></div>
