...
async function read<T>(path: string, signal: AbortSignal): Promise<T> {
  ...
  if (!response.ok) throw new FrontierReadError(response.status, typeof body?.detail === "string" ? body.detail : "Frontier read failed.")
  return body as T
}
const collectionLink = (id: string) => `/suggest?request=${encodeURIComponent(id)}#request`
const itemLink = (id: string) => `/frontier/${encodeURIComponent(id)}`
function modeLabel(mode: string) { return mode === "acquired" ? "Acquisition requested" : mode === "shared" ? "Shared work" : mode === "reused" ? "Recent result reuse" : mode }
...
export function CollectionItems({ id }: { id: string }) {
  ...
  return <section className="flex min-w-0 flex-col gap-4"><h2>Request frontier items</h2><p className="text-sm text-muted-foreground">Current retained work, ordered by identity, not dispatch position. Newly admitted items may appear on earlier pages; refresh from the beginning to review them.</p>
    ...
    {query.error && <p role="alert">{query.data ? "Frontier items may be stale. " : "Current frontier items unavailable. "}{extractApiError(query.error)}</p>}
    ...
    {query.data?.items.map(item => <Card key={item.interest_id}><CardHeader><CardTitle><a className="break-all underline" href={itemLink(item.acquisition.id)}>{item.acquisition.url}</a></CardTitle></CardHeader><CardContent className="flex flex-col gap-2"><p>Request work: {item.status.replaceAll("_", " ")} · {modeLabel(item.mode)} · Page unit {item.budget_state}</p><p>Depth {item.context.depth} · Rule {item.context.rule_id} · Acquisition {item.acquisition.status}</p>{item.acquisition.waiting_reason && <p>{item.acquisition.waiting_reason.replaceAll("_", " ")}</p>}{item.context.parent_observation_id && <Observation id={item.context.parent_observation_id} label="Parent observation" />}{item.acquisition.observation_id && <><Observation id={item.acquisition.observation_id} label="Observation" /><Readiness item={item.acquisition} /></>}<p className="text-sm text-muted-foreground">Admitted {new Date(item.admitted_at).toLocaleString()}</p></CardContent></Card>)}
...
export function FrontierItemDetail({ id }: { id: string }) {
  ...
  return <div className="flex w-full min-w-0 flex-col gap-4"><h1 className="text-2xl font-semibold">Frontier item</h1><Button className="self-start" variant="outline" disabled={query.isFetching} onClick={() => void query.refetch()}>Refresh frontier item</Button>
    ...
    {query.error && !missing && <p role="alert">{item ? "Frontier status may be stale. " : "Current frontier item unavailable. "}{extractApiError(query.error)}</p>}
    {item && <><p className="break-all">{item.url}</p><div><Badge>{item.status}</Badge></div><p>{item.domain} · {item.attempt_count} physical attempts started</p><p className="text-sm text-muted-foreground">As of {new Date(item.as_of).toLocaleString()} · Created {new Date(item.created_at).toLocaleString()}</p>
      {item.waiting_reason && <p>Current stage: {item.waiting_reason.replaceAll("_", " ")}</p>}
      {item.eligibility_not_before && <p>Eligibility floor: {new Date(item.eligibility_not_before).toLocaleString()}. This is not a promised start time; domain and dispatch constraints still apply.</p>}
      ...
      <p>Evidence commit: {item.evidence_committed ? "confirmed" : "not yet confirmed"}.</p><Readiness item={item} />
      {item.observation_id && <Observation id={item.observation_id} label="Query observation" />}
      <Card><CardHeader><CardTitle>Requesting collections</CardTitle></CardHeader><CardContent className="flex flex-col gap-3"><p className="text-sm text-muted-foreground">Visible current associations. Retired collection lineage remains in the catalogue.</p>{item.callers.map(caller => <div key={caller.interest_id}><a className="break-all underline" href={collectionLink(caller.collection_id)}>{caller.collection_id}</a><p>{modeLabel(caller.mode)} · {caller.status.replaceAll("_", " ")}</p></div>)}{item.more_callers && <p>More visible associations exist; this preview shows ten.</p>}{!item.callers.length && <p>No visible current collection associations.</p>}</CardContent></Card>
...
export function CollectionArrivals({ id }: { id: string }) {
  ...
  return <section className="flex min-w-0 flex-col gap-4"><h2>Durable arrivals</h2><p className="text-sm text-muted-foreground">Fulfillments recorded in the catalogue, newest decisions first. They remain available after current frontier work is retired. Refresh from the newest page to include later commits.</p>
    ...
    {query.data?.items.map(item => <Card key={item.fulfillment_id}><CardHeader><CardTitle><span className="break-all">{item.requested_url}</span></CardTitle></CardHeader><CardContent className="flex flex-col gap-2"><p>{item.mode === "acquired" ? "Acquired result" : item.mode === "shared" ? "Shared result" : "Reused result"} · Depth {item.depth} · Rule {item.rule_id}</p><p>Fulfillment recorded {new Date(item.decided_at).toLocaleString()}</p><p>{item.observation_committed ? `Observation committed: ${item.outcome ?? "outcome unavailable"}${item.http_status_code === null ? "" : ` · HTTP ${item.http_status_code}`}` : "Observation commit not yet confirmed."}</p>{item.effective_url && <p className="break-all">Effective URL: {item.effective_url}</p>}<p>{item.query_ready === true ? "Query readiness verified for the active generation." : item.query_ready === false ? "Materialization is still pending." : "Query readiness has not been verified."}</p><Observation id={item.observation_id} label="Query observation" />{item.parent_observation_id && <Observation id={item.parent_observation_id} label="Traversal parent" />}</CardContent></Card>)}
