...
export function publicCollectionSpec(input: { kind: "url" | "description"; input: string; depth: number; scope: "internal" | "external" | "both"; maxPages: number; maxLinks: number; sections: string; retentionSeconds?: number | null }): CollectionSpec {
  ...
  if (!Number.isInteger(input.maxLinks) || input.maxLinks < 1 || input.maxLinks > 10000) throw new Error("Choose a link limit from 1 to 10,000.")
  ...
  return { seed_urls: input.kind === "url" ? [value] : [], seed_description: input.kind === "description" ? value : null, seed_sql: null, seed_parameters: [], follow_sql, max_depth: input.depth, follow_link_limit: input.maxLinks, page_limit: input.maxPages, result_max_age_seconds: 300, retention_seconds: input.retentionSeconds ?? null, request_class: "public", allowed_sections: sections, max_duration_seconds: null }
}
