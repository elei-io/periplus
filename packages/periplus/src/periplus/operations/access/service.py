...
class AccessStore:
    ...
    def admit(self, capability: Capability, *, specification: CollectionSpec | None = None, consume: bool = True, now: datetime | None = None) -> None:
        with self.sessions.begin() as session:
            self.admit_in_session(session, capability, specification=specification, consume=consume, now=now)

    def admit_in_session(self, session: Session, capability: Capability, *, specification: CollectionSpec | None = None, consume: bool = True, now: datetime | None = None) -> None:
        """Apply public policy within the transaction that creates scheduled intent."""
        row = session.get(PublicAccessRecord, 1, with_for_update=True)
        if row is None:
            raise AccessDenied("access_unavailable", "Public access settings are unavailable.", 503)
        policy = AccessPolicy.model_validate(row.configuration)
