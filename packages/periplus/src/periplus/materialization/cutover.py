...
from periplus.platform.catalogue.public import validate_public_catalogue
...
def verify_cutover(catalogue: Catalogue) -> dict[str, int]:
    ...
    catalogue.trusted_remote_rows("SELECT * FROM web.observation LIMIT 1")
    catalogue.trusted_remote_rows("SELECT * FROM content.object LIMIT 1")
    return counts
