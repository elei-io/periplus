...
from periplus.crawl.runtime.frontier_outbox import run_outbox_relay
from periplus.crawl.runtime.frontier_selection import (
    PolicyResolver,
    process_link_selection,
    process_seed_selection,
)
...
def service_collection(
    ...
    *,
    ...
    if collection.deadline_at is not None and collection.deadline_at <= datetime.now(
        UTC
    ):
    ...
    if not collection.seeds_settled:
        result = process_seed_selection(
            store, collection.id, policy, seed_query=seed_query
        )
...
async def _service_collection(
    ...
    if collection is not None and collection.status == "active":
        ...
        if (
            ...
            and collection.selection_checkpoint is None
            and (
                collection.deadline_at is None
                or collection.deadline_at > datetime.now(UTC)
            )
        ):
            ...
            if not state.complete:
                ...
                async with asyncio.timeout(30):
                    selected = await discovery.step(
                        spec.seed_description, spec.page_limit, state
                    )
                await asyncio.to_thread(
                    store.checkpoint_discovery, work, state.revision, selected
                )
                return 0
    return await asyncio.to_thread(
        service_collection, store, work, policy, objects, seed_query=seed_query
    )
...
async def run_collection_selection(
    store: FrontierStore,
    policy: PolicyResolver,
    objects: ObjectStore,
    *,
    ...
    while not stop.is_set():
        ...
        try:
            ...
            await asyncio.to_thread(
                store.release_collection, work, error="discovery_dependency_unavailable"
            )
        except ValueError:
            logger.exception(
                "invalid collection selection collection=%s", work.collection_id
            )
            await asyncio.to_thread(
                store.stop_collection, work.collection_id, reason="failed"
            )
            await asyncio.to_thread(store.release_collection, work)
        except Exception:
            logger.exception(
                "collection selection deferred collection=%s", work.collection_id
            )
            await asyncio.to_thread(
                store.release_collection, work, error="selection_dependency_unavailable"
            )
        else:
...
async def run_dispatch(
    ...
    *,
    ...
    playwright,
    stop: asyncio.Event,
    health: DispatchHealth | None = None,
) -> None:
    ...
    while not stop.is_set():
        if time.monotonic() >= healthy_until:
            ...
            try:
                ...
                logger.warning(
                    "frontier dispatch waiting for acquisition dependencies",
                    exc_info=True,
                )
                await _wait(stop, 30)
                continue
...
async def run_frontier(
    ...
    *,
    ...
    seed_query: Callable[[QueryRequest], SelectionCheckpoint] | None = None,
    discovery: SourceDiscovery | None = None,
    dispatch_health: DispatchHealth | None = None,
) -> None:
    ...
    async with asyncio.TaskGroup() as tasks:
        ...
        tasks.create_task(
            run_outbox_relay(store, jetstream, ingestion, stop=stop),
            name="frontier-outbox",
        )
        tasks.create_task(
            run_collection_selection(
                store,
                policy,
                pipeline.html_repository.store,
                stop=stop,
