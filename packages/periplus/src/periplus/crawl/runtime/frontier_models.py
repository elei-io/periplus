...
class FrontierControlRecord(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "pending_count >= 0 AND active_count >= 0 AND interest_count >= 0",
...
class AcquisitionRecord(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "status IN ('queued', 'retry', 'dispatched', 'succeeded', 'failed', 'cancelled')",
    ...
    attempt_exclusions: Mapped[list[dict[str, Any]]] = mapped_column(
    ...
    uncertain_attempts: Mapped[list[dict[str, Any]]] = mapped_column(
        json_type, default=list
    )
    prior_results: Mapped[list[dict[str, Any]]] = mapped_column(json_type, default=list)
    terminal_reason: Mapped[str | None] = mapped_column(Text)
    frozen_reasons: Mapped[list[dict[str, Any]]] = mapped_column(
        json_type, default=list
    )
    ...
    evidence_committed_at: Mapped[datetime | None] = mapped_column(
...
class InterestRecord(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "collection_id", "url_key", name="uq_frontier_collection_url_key"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "budget_state IN ('reserved', 'consumed', 'released')",
