...
def acquisition_context(acquisition) -> AcquisitionContext:
    previous = [
        AcquisitionResult.model_validate(value) for value in acquisition.prior_results
    ]
    attempts = [
        result.attempt_evidence
        for result in previous
        if result.attempt_evidence is not None
    ]
    for uncertain in acquisition.uncertain_attempts:
        attempts.append(
            AcquisitionAttemptEvidence(
                attempt=0,
                started_at=uncertain["started_at"],
                completed_at=None,
                requested_url=acquisition.url,
                outcome="uncertain",
                resource_usage=uncertain["resource_usage"],
            )
        )
    attempts.sort(key=lambda attempt: attempt.started_at)
    attempts = tuple(
        attempt.model_copy(update={"attempt": index + 1})
    ...
    if acquisition.attempt_domain_policy is not None:
        ...
        policy = policy.model_copy(
            update={
                "domain": DomainPolicySnapshot.model_validate(
    ...
    return AcquisitionContext(
        ...
        prior_steps=tuple(step for result in previous for step in result.steps),
    )
...
def terminal_evidence(acquisition, now: datetime, outcome: str) -> VisitEvidence:
    ...
    return VisitEvidence(
        ...
        attempts=attempts,
        steps=step_records(acquisition.id, context.prior_steps),
    )
