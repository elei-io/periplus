...
from periplus.crawl.runtime.collection_queue import (
    CollectionQueue,
    collection_queue,
    _aware,
)
    ...
    with sessions() as session:
        ...
        if request_class is not None:
            statement = statement.where(
                CollectionRecord.spec["request_class"].as_string() == request_class
            )
        records = list(
            session.scalars(
                statement.order_by(
                    CollectionRecord.created_at.desc(), CollectionRecord.id.desc()
                )
                .limit(limit)
                .offset(offset)
            )
        ...
        views = {
            record.id: CollectionView(
                id=record.id,
                specification=CollectionExecutionSpec.model_validate(record.spec),
                status=record.status,
                priority=record.priority,
                reserved_pages=record.reserved,
                consumed_pages=record.consumed,
                seeds_settled=record.seeds_settled,
                ...
                admission=admission_wait(
                    session, record, now=as_of, control=control, workers=workers
                ),
                queue=collection_queue(
                    session, record, control, workers=workers, now=as_of
                ),
                last_progress_at=max(
                    _aware(value)
                    for value in (
                        record.last_progress_at,
                        record.last_dispatch_at,
                        record.completed_at,
                        record.created_at,
                    )
        ...
        for record in records:
            if record.spec.get("seed_description"):
                ...
                view.discovery_stage = (
                    "complete"
                    if state.complete
                    else "planning"
                    if not state.queries
                    else "searching"
                    if len(state.searches) < len(state.queries)
                    else "selecting"
        ...
        if not views:
            return []
        outcome_status = func.coalesce(
            AcquisitionRecord.status, InterestRecord.completed_status
        )
        rows = session.execute(
            select(
                InterestRecord.collection_id,
                InterestRecord.status,
                InterestRecord.mode,
                outcome_status,
                func.count(),
            )
            ...
            .where(InterestRecord.collection_id.in_(views))
            .group_by(
                InterestRecord.collection_id,
                InterestRecord.status,
                InterestRecord.mode,
        ...
        for collection_id, count in session.execute(
            ...
            .where(
                ...
                (
                    AcquisitionRecord.evidence_committed_at.is_not(None)
                    | InterestRecord.completed_evidence.is_(True)
                ),
        ...
        for identity, view in views.items():
            if (
                view.status == "active"
                and view.waiting_reason is None
                and not view.acquiring_pages
                and not view.selecting_pages
                and view.queue.deferred_pages
                and not view.queue.runnable_pages
                and not view.queue.unknown_pages
                and len(view.queue.constraints) == 1
            ):
                view.waiting_reason = view.queue.constraints[0].reason
            view.lineage_ready = view.status == "settled"
            if (
                view.lineage_ready
                and view.ingested_pages == view.supplied_pages + view.failed_pages
            ):
        ...
        result.append(
            view.model_copy(
                update={
                    ...
                    if proof
                    else "catalogue_readiness_unavailable",
