...
# unobserved interval approaching that boundary; capture facts stay in the raw archive.
...
def decode_capture_cursor(value: str | None) -> CaptureCursor | None:
    ...
    try:
        ...
        cursor = CaptureCursor.model_validate_json(
            base64.b64decode(
                value + "=" * (-len(value) % 4), altchars=b"-_", validate=True
            )
        )
        dates = [cursor.after_time, cursor.expires_at] + (
            [cursor.through_time] if cursor.through_time else []
        )
...
def capture_page(sessions, cursor: CaptureCursor | None = None) -> CapturePage:
    with sessions() as session:
        ...
        statement = select(
            ...
            AcquisitionRecord.completed_at <= upper,
        )
        if cursor is None:
            rows = list(
                reversed(
                    session.execute(
                        statement.order_by(
                            AcquisitionRecord.completed_at.desc(),
                            AcquisitionRecord.id.desc(),
                        ).limit(7)
                    ).all()
                )
            )
            ...
            statement = statement.where(
                or_(
                    AcquisitionRecord.completed_at > cursor.after_time,
                    and_(
                        AcquisitionRecord.completed_at == cursor.after_time,
                        AcquisitionRecord.id > cursor.after_id,
                    ),
                )
            )
            rows = session.execute(
                statement.order_by(
                    AcquisitionRecord.completed_at, AcquisitionRecord.id
                ).limit(_PAGE_SIZE + 1)
            ).all()
        ...
        following = CaptureCursor(
            after_time=_aware(rows[-1][2]) if has_more else upper,
            after_id=rows[-1][0] if has_more else _MAX_ID,
            through_time=upper if has_more else None,
            expires_at=upper + _CURSOR_TTL,
        )
        encoded = (
            base64.urlsafe_b64encode(following.model_dump_json().encode())
            .decode()
            .rstrip("=")
        )
        return CapturePage(
