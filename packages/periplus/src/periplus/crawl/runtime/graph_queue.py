...
from periplus.platform.config.performance import GRAPH_ACK_WAIT_SECONDS, GRAPH_CONSUMER_MAX_ACK_PENDING
...
from periplus.platform.messaging.topology import list_kv_keys

GRAPH_STREAM = "PERIPLUS_GRAPH_WORK"
CRAWL_SUBJECT = "periplus.graph.crawl"
EDGE_SUBJECT = "periplus.graph.edge"
NAVIGATION_READINESS_SUBJECT = "periplus.graph.navigation.readiness"
CRAWL_CONSUMER = "periplus-graph-crawl-workers"
EDGE_CONSUMER = "periplus-graph-edge-workers"
NAVIGATION_READINESS_CONSUMER = "periplus-graph-navigation-readiness-workers"
WORKERS_BUCKET = "periplus_graph_workers"
...
async def _bucket(jetstream, config: KeyValueConfig):
    ...
    if (
        ...
        raise RuntimeError(
            ...
            "reset disposable NATS state before starting Periplus"
        )
...
async def ensure_graph_storage(jetstream):
    replicas = get_int("PERIPLUS_GRAPH_STREAM_REPLICAS")
    subjects = [CRAWL_SUBJECT, EDGE_SUBJECT, NAVIGATION_READINESS_SUBJECT]
    max_bytes = get_int("PERIPLUS_GRAPH_WORK_MAX_BYTES")
    stream = StreamConfig(
        name=GRAPH_STREAM,
        subjects=subjects,
    ...
    workers = await _bucket(
        ...
        KeyValueConfig(
            ...
            description="Ephemeral Periplus crawler presence",
            history=1,
            ttl=get_float("PERIPLUS_CRAWLER_PRESENCE_TTL_SECONDS"),
            max_bytes=get_int("PERIPLUS_GRAPH_WORKER_MAX_BYTES"),
            storage=StorageType.FILE,
            replicas=replicas,
        ),
    )
