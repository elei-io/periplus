...
async def item(identity: UUID, request: Request):
    ...
    return (await enrich_readiness([value], request.app.state.crawl_results))[0]
...
async def live(request: Request, collection_id: UUID | None = None):
    ...
    try:
        history = await request.app.state.crawl_results.live(now=current.as_of)
...
async def observation_lineage(identity: UUID, request: Request,
    ...
    try:
        page = await request.app.state.crawl_results.observation_lineage(identity,
