...
from periplus.crawl.runtime.frontier_items import (
    CollectionItemsPage,
    collection_items,
    enrich_readiness,
)
from periplus.crawl.runtime.frontier_views import (
    CollectionView,
    collection_views,
    enrich_collection_readiness,
)
...
async def _views(request: Request, **kwargs):
    ...
    return await asyncio.to_thread(
        collection_views, request.app.state.frontier_sessions, workers=workers, **kwargs
    )
...
async def create(payload: CreateCollection, request: Request):
    ...
    if not spec.seed_urls and not spec.seed_sql and not spec.seed_description:
        raise HTTPException(
            422, "Provide starting URLs, a source description, or seed SQL."
        )
    try:
    ...
    if views:
        return (
            await enrich_collection_readiness(views, request.app.state.crawl_results)
        )[0]
...
async def list_collections(
    request: Request,
    status: Literal["active", "paused", "settled"] | None = None,
    request_class: Literal["public", "system", "admin"] | None = None,
    limit: Annotated[int, Query(ge=1, le=100)] = 20,
    offset: Annotated[int, Query(ge=0, le=10000)] = 0,
):
    views = await _views(
        request, status=status, request_class=request_class, limit=limit, offset=offset
    )
...
async def history_page(
    ...
    try:
        ...
        raise HTTPException(
            503,
            "Collection history is unavailable; retry later.",
...
async def act(identity: UUID, payload: CollectionAction, request: Request):
    ...
    try:
        if payload.action == "cancel":
            await asyncio.to_thread(
                request.app.state.frontier.stop_collection, identity
            )
        else:
            await asyncio.to_thread(
                request.app.state.frontier.set_collection_paused,
                identity,
...
async def prioritize(identity: UUID, payload: CollectionPriority, request: Request):
    ...
    try:
        await asyncio.to_thread(
            request.app.state.frontier.set_collection_priority,
            identity,
            payload.priority,
        )
...
async def items(
    ...
    page = await asyncio.to_thread(
        collection_items,
        request.app.state.frontier_sessions,
        identity,
        limit=limit,
        after=after,
        workers=workers,
    ...
    if page is None:
        raise HTTPException(
            404,
            "Current collection not found; use its durable history for historical arrivals.",
        )
    enriched = await enrich_readiness(
        [item.acquisition for item in page.items], request.app.state.crawl_results
    )
    return page.model_copy(
        update={
            "items": [
                item.model_copy(update={"acquisition": acquisition})
...
async def arrivals(
    ...
    try:
        page = await request.app.state.crawl_results.arrivals(
        ...
        raise HTTPException(
            503,
            "Collection arrivals are unavailable; retry later.",
            headers={"Retry-After": "5"},
        ) from exc
    if page is not None:
        return page
