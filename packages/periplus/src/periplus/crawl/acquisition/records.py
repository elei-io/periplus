"""Frozen native acquisition records used by frontier execution."""
...
_LINK_OCCURRENCE_NAMESPACE = UUID("f48cf2ce-fdd6-5d20-a6ef-dddc3ac70419")
...
def link_occurrence_id_for(document_id: UUID, element_index: int) -> UUID:
    if element_index < 0:
...
class VisitRecord(AcquisitionRecord):
    ...
    archive_source: ArchiveSource | None = Field(
    ...
    def validate_record(self) -> VisitRecord:
        ...
        if self.archive_source is not None:
            if (
                ...
                or self.visit_id != self.archive_source.capture_id
            ):
                raise ValueError(
                    "archive observations require source identity and no native policy"
                )
            if (
                self.outcome != "succeeded"
                or self.document_id is None
                or self.observed_at is None
                or not self.admitted_at
                == self.started_at
                == self.observed_at
                == self.finished_at
            ):
                raise ValueError(
                    "archive observations require a document at one original capture timestamp"
                )
        ...
        if (self.document_id is not None) != (self.observed_at is not None):
            raise ValueError("document_id and observed_at must be present together")
...
class AttemptUsage(AcquisitionRecord):
...
class AttemptRecord(AcquisitionRecord):
    ...
    def validate_record(self) -> AttemptRecord:
        if self.attempt_id != attempt_id_for(self.visit_id, self.attempt_index):
            raise ValueError(
                "attempt_id must be derived from visit_id and attempt_index"
            )
...
class StepRecord(AcquisitionRecord):
    attempt_id: UUID
...
class DocumentRecord(AcquisitionRecord):
    document_id: UUID
    visit_id: UUID
...
class VisitEvidence(AcquisitionRecord):
    visit: VisitRecord
        ...
        if self.document is not None:
            if self.visit.archive_source is not None:
                if self.attempts or self.steps or self.document.attempt_id is not None:
                    raise ValueError(
                        "archive observations cannot invent native attempts"
                    )
