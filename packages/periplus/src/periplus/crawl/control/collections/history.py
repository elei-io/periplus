...
def read_collection(catalogue, identity: UUID, *, public_only: bool) -> HistoricalCollection | None:
    ...
    try:
        rows = connection.execute(f"""
            SELECT d.collection_id,
                   ...
                   octet_length(encode(CAST(o.seed_provenance AS VARCHAR))), d.visibility
            FROM {alias}.ingest.collections d
            LEFT JOIN {alias}.ingest.collection_outcomes o
              ON o.collection_id = d.collection_id AND o.visibility = d.visibility
            WHERE d.collection_id = ? AND (NOT ? OR d.visibility = 'public')
            LIMIT 2
        ...
        if specification.get("visibility", row[10]) != row[10]:
            raise ValueError("historical collection visibility is inconsistent")
        specification = specification | {"visibility": row[10]}
...
class CollectionHistory:
    ...
    async def get(self, identity: UUID, *, public_only: bool) -> HistoricalCollection | None:
        return await self._read(lambda catalogue: read_collection(catalogue, identity, public_only=public_only))

    async def list(self, *, public_only: bool, limit: int, cursor: str | None):
        anchor = decode_cursor(cursor)
        return await self._read(lambda catalogue: read_history_page(catalogue, public_only=public_only, limit=limit, cursor=anchor))
    ...
    async def collection_readiness(self, identities: list[UUID], *, public_only: bool):
    ...
    async def readiness(self, identities: list[UUID], *, public_only: bool):
    ...
    async def observation_lineage(self, identity: UUID, *, public_only: bool, limit: int, cursor: str | None):
        ...
        anchor = decode_lineage_cursor(cursor, identity, public_only=public_only)
        return await self._read(lambda catalogue: read_observation_lineage(catalogue, identity,
            public_only=public_only, limit=limit, cursor=anchor))

    async def arrivals(self, identity: UUID, *, public_only: bool, limit: int, cursor: str | None):
...
def read_history_page(catalogue, *, public_only: bool, limit: int, cursor: HistoryCursor | None) -> CollectionHistoryPage:
    if not 1 <= limit <= 100:
    ...
    try:
        ...
        rows = connection.execute(f"""
            SELECT d.collection_id, d.visibility,
                   ...
                   CASE WHEN octet_length(encode(CAST(d.specification AS VARCHAR))) <= 1048576
                     THEN coalesce(json_extract_string(d.specification, '$.visibility'), d.visibility)
                     ELSE NULL END
            FROM {alias}.ingest.collections d
            LEFT JOIN {alias}.ingest.collection_outcomes o
              ON o.collection_id = d.collection_id AND o.visibility = d.visibility
            WHERE (NOT ? OR d.visibility = 'public') {anchor}
        ...
        for row in rows:
            ...
            if row[9] != row[1]:
                raise ValueError("historical collection visibility is inconsistent")
        items = [HistoricalCollectionSummary(id=row[0], visibility=row[1], summary=row[2], created_at=row[3],
