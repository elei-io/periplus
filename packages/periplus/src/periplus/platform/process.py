"""Shared process-lifecycle mechanics for Periplus worker entrypoints."""
...
class WorkerEndpointConfig:
    ...
    def from_env(cls, role: WorkerRole) -> WorkerEndpointConfig:
        prefix = f"PERIPLUS_{role.upper().replace('-', '_')}"
        metrics_enabled = get_bool("PERIPLUS_METRICS_ENABLED")
        return cls(
            health_address=get_str(f"{prefix}_HEALTH_HOST"),
            health_port=get_int(f"{prefix}_HEALTH_PORT"),
            metrics_enabled=metrics_enabled,
            metrics_address=get_str("PERIPLUS_METRICS_HOST"),
            metrics_port=(
