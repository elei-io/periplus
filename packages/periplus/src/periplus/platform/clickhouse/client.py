...
class ClickHouseConfig(BaseModel):
    ...
    def validate_url(cls, value: str) -> str:
        ...
        if (endpoint.scheme not in {"http", "https"} or not endpoint.hostname
            ...
            raise ValueError("ClickHouse URL must be an HTTP(S) origin without credentials")
    ...
    def for_query(cls) -> "ClickHouseConfig":
        return cls(url=get_str("PERIPLUS_CLICKHOUSE_URL"),
                   username=get_str("PERIPLUS_CLICKHOUSE_QUERY_USER"),
                   password=SecretStr(get_str("PERIPLUS_CLICKHOUSE_QUERY_PASSWORD")), query_only=True)
...
class ClickHouseError(RuntimeError):
    ...
    def __init__(self, query_id: str, *, code: str | None = None) -> None:
        ...
        self.code = code
        super().__init__(f"ClickHouse operation failed (query_id={query_id}, code={code or 'unknown'})")
...
class ClickHouseClient:
    ...
    def __init__(self, config: ClickHouseConfig, *, transport: httpx.BaseTransport | None = None) -> None:
        ...
        self._http = httpx.Client(
            ...
            limits=httpx.Limits(max_connections=1, max_keepalive_connections=0 if config.query_only else 1),
            transport=transport or httpx.HTTPTransport(retries=0, limits=httpx.Limits(
                max_connections=1, max_keepalive_connections=0 if config.query_only else 1)),
            follow_redirects=False,
            trust_env=False,
        )

    def execute(self, sql: str, *, parameters: Mapping[str, str] | None = None,
                data: bytes = b"", query_id: str | None = None,
                max_response_bytes: int = 8 * 1024 * 1024,
                timeout_seconds: float = 45, cancelled: threading.Event | None = None) -> bytes:
        ...
        params = {
            "query": sql, "query_id": identity, "wait_end_of_query": "1",
            "max_execution_time": str(timeout_seconds), "async_insert": "0",
            "date_time_input_format": "best_effort",
            "output_format_json_quote_64bit_integers": "0",
            **{f"param_{key}": value for key, value in (parameters or {}).items()},
        }
        if self._query_only:
            for setting in ("async_insert", "date_time_input_format", "output_format_json_quote_64bit_integers"):
                params.pop(setting)
        if not data:
            data = sql.encode()
            params.pop("query")
        ...
        if cancelled is not None and cancelled.is_set():
            raise TimeoutError("Query was cancelled.")
        try:
            with self._http.stream("POST", "/", params=params, content=data,
                                   extensions={"trace": trace} if self._query_only else {},
                                   timeout=httpx.Timeout(timeout_seconds + 1, connect=min(5, timeout_seconds), pool=5)) as response:
                if response.status_code != 200 or "X-ClickHouse-Exception-Code" in response.headers:
                    raise ClickHouseError(identity, code=response.headers.get("X-ClickHouse-Exception-Code"))
                result = bytearray()
    ...
    def interrupt_query(self) -> None:
        ...
        if not self._query_only:
            raise RuntimeError("Write requests cannot be cancelled as read-only queries.")
        with self._socket_lock:
    ...
    def query(self, sql: str, *, parameters: Mapping[str, str] | None = None) -> dict[str, JsonValue]:
        result = json.loads(self.execute(sql + " FORMAT JSON", parameters=parameters))
    ...
    def insert_json(self, table: str, row: Mapping[str, JsonValue]) -> None:
        ...
        structure = ", ".join(f"{key} {columns[key].replace('FixedString(32)', 'String')}" for key in row)
