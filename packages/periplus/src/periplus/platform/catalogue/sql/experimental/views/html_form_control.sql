CREATE OR REPLACE VIEW experimental.html_form_control AS
WITH controls AS NOT MATERIALIZED (
    ...
    FROM controls c
    LEFT JOIN experimental.html_element ancestor ON ancestor.content_id = c.content_id
...
SELECT c.content_id, c.node_index, o.form_node_index, c.tag,
       ...
       CASE WHEN c.tag = 'textarea' THEN c.text_direct ELSE c.attributes['value'] END AS value,
