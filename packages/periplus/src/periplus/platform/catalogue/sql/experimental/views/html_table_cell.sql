...
CREATE OR REPLACE VIEW experimental.html_table_cell AS
WITH rows AS NOT MATERIALIZED (
    ...
    FROM experimental.html_element r
    JOIN experimental.html_element p ON p.content_id = r.content_id AND p.node_index = r.parent_index
    JOIN experimental.html_element t ON t.content_id = r.content_id
