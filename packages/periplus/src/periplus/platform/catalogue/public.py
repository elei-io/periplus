...
PUBLIC_CATALOGUE_VERSION = "1.1.0"
WEB_SCHEMA = "web"
...
def install_public_catalogue(
    ...
    *,
    ...
    root = files("periplus.platform.catalogue").joinpath("sql")
    boundary = catalogue.remote_transaction() if transaction else nullcontext()
    with boundary:
        for schema in RETIRED_PUBLIC_SCHEMAS:
...
def validate_public_catalogue(catalogue: CatalogueConnection) -> None:
    ...
    for schema in RETIRED_PUBLIC_SCHEMAS:
        retired_views = catalogue.trusted_remote_rows(
            "SELECT view_name FROM duckdb_views() "
            f"WHERE schema_name = {_quote_literal(schema)}"
        )
        retired_macros = catalogue.trusted_remote_rows(
            "SELECT function_name FROM duckdb_functions() "
            f"WHERE schema_name = {_quote_literal(schema)} "
            "AND function_type IN ('macro', 'table_macro')"
        )
        if retired_views or retired_macros:
            errors.append(f"retired public schema {schema} is not empty")
