...
def query_error(error: Exception) -> tuple[int, QueryError]:
    if isinstance(error, ClickHouseError):
        if error.code in {"159", "394"}:
            return 408, QueryError(code="resource_limit", detail="Query execution exceeded its deadline or was cancelled.")
        if error.code in {"241", "202", "396", "response_limit"}:
            return 422, QueryError(code="resource_limit", detail="Query exceeded its execution or result budget.")
        if error.code in {"62", "47", "43", "44", "46", "53", "164", "497"}:
            return 422, QueryError(code="sql_invalid", detail="Query could not be evaluated against the public ClickHouse catalogue.")
        return 503, QueryError(code="storage_unavailable", detail="ClickHouse could not complete the read; retry after the service recovers.")
