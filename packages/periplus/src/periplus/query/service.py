...
def public_sql(payload: QueryRequest) -> str:
    ...
    try:
        tokens = sqlglot.Dialect.get_or_raise('clickhouse').tokenize(payload.sql)
        ...
        sql = payload.sql
        for token, value in reversed(list(zip(placeholders, payload.parameters, strict=True))):
...
class QueryService:
    def __init__(self, config: ClickHouseConfig, *, deadline: float | None = None,
                 mode: QueryMode = QueryMode.STABLE):
        if mode != QueryMode.STABLE or not config.query_only:
            raise ValueError('Query service requires public_v1 and query-only credentials.')
        self.mode = mode
        ...
        try:
            for relation in sorted(PUBLIC_RELATIONS):
                self.client.execute(f'SELECT * FROM public_v1.{relation} LIMIT 0')
    ...
    def _run(self, payload, limits, evidence, *, execute, emit=None, cancelled=None):
        ...
        def remaining():
            ...
            if value <= 0 or self._cancelled.is_set() or (cancelled is not None and cancelled.is_set()):
                raise TimeoutError('Query execution was cancelled or exceeded its deadline.')
            return value
        try:
            if self._closed:
            ...
            prepared = PreparedQuery(query_mode=self.mode, compiler_version=self.compiler_version,
                schema_version=self.schema, query_id=query_id, sql=payload.sql, parameters=payload.parameters,
            ...
            body = self.client.execute(f'SELECT * FROM ({sql}) LIMIT {limits.max_rows + 1} FORMAT JSONCompact',
                query_id=query_id, cancelled=self._cancelled, timeout_seconds=remaining(),
                max_response_bytes=min(16 * 1024 * 1024, limits.max_result_bytes + 1024 * 1024))
            remaining()
            ...
            for native_row in result['data']:
                ...
                if len(rows) == limits.max_rows:
                    reason = 'max_rows'
                    break
                count = len(json.dumps(row, ensure_ascii=False, allow_nan=False).encode()) + 1
                if size + count > limits.max_result_bytes:
                    reason = 'max_result_bytes'
                    break
                rows.append(row)
                size += count
            answer = QueryResult(**prepared.model_dump(), columns=columns, types=types, rows=rows,
                truncated=reason is not None, truncation_reason=reason, source_snapshot=None,
                elapsed_ms=(time.monotonic()-started)*1000, row_count=len(rows), result_bytes=size)
            if emit:
                emit(dict(type='metadata', **prepared.model_dump(mode='json'), columns=columns, types=types,
                          source_snapshot=None, limits=limits.model_dump()))
