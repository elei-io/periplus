...
class TermMaterializationTests(unittest.TestCase):
    ...
    def test_node_postings_commit_replay_and_replace_atomically(self):
        ...
        self.assertEqual(self.rows("SELECT t.text,p.frequency FROM material.content_posting p JOIN material.term t USING(term_id)"), [('monkey', 2)])
        self.assertEqual(self.rows("SELECT t.text,p.node_index,p.frequency FROM material.node_posting p JOIN material.term t USING(term_id) ORDER BY node_index"), initial)
