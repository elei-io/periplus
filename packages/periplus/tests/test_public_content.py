    ...
    def test_retained_hash_download_and_missing_evidence(self):
        with TemporaryDirectory() as directory, duckdb.connect() as db:
            ...
            with TestClient(app) as client:
                ...
                db.execute("UPDATE ingest.documents SET detected_media_type='application/pdf'")
                self.assertEqual(client.get(f'/documents/by-content/{digest}/content').status_code, 404)
                self.assertEqual(client.get(f'/documents/{document_id}/content').status_code, 200)
                db.execute("UPDATE ingest.documents SET detected_media_type='TEXT/HTML'")
                self.assertEqual(client.get(f'/documents/by-content/{digest}/content').status_code, 200)
