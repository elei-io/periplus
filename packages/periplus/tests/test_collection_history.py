...
class CollectionHistoryTests(unittest.IsolatedAsyncioTestCase):
    async def test_busy_history_is_bounded_and_cancelled_reader_drains_before_releasing_slot(self):
        ...
        reading = asyncio.create_task(history.get(uuid4()))
        await entered.wait()
        with self.assertRaises(HistoryUnavailable):
            await history.get(uuid4())
        reading.cancel()
    ...
    async def test_overlapping_page_reads_serialize_with_bounded_admission(self):
        ...
        control.run.side_effect = run
        history = CollectionHistory(control)
        tasks = [asyncio.create_task(history.get(uuid4())) for _ in range(8)]
        await entered.wait()
        await asyncio.sleep(0)
    ...
    async def test_cancelled_waiter_does_not_release_the_active_reader(self):
        ...
        control.run.side_effect = run
        history = CollectionHistory(control)
        active = asyncio.create_task(history.get(uuid4()))
        await entered.wait()
        queued = asyncio.create_task(history.get(uuid4()))
        await asyncio.sleep(0)
        queued.cancel()
        with self.assertRaises(asyncio.CancelledError):
            await queued
    ...
    async def test_storage_failure_is_not_an_absent_identity(self):
        ...
        with self.assertRaises(HistoryUnavailable):
            await CollectionHistory(control).get(uuid4())
    ...
    async def test_listing_shares_read_slot_and_invalid_cursor_does_not_use_catalogue(self):
        ...
        for cursor in ("!", "x" * 513, base64.urlsafe_b64encode(
            ...
            with self.assertRaises(ValueError):
                await history.list( limit=20, cursor=cursor)
        control.run.assert_not_awaited()
        async with history.slot:
            with self.assertRaises(HistoryUnavailable):
                await history.list( limit=20, cursor=None)
    ...
    async def test_arrival_reads_share_the_slot_and_validate_cursor_scope_before_io(self):
        ...
        with self.assertRaises(ValueError):
            await history.arrivals(identity,  limit=20, cursor=cursor)
        with self.assertRaises(ValueError):
            await history.arrivals(identity,  limit=101, cursor=None)
        control.run.assert_not_awaited()
        async with history.slot:
            with self.assertRaises(HistoryUnavailable):
                await history.arrivals(identity,  limit=20, cursor=None)

    async def test_readiness_is_bounded_before_io_and_shares_history_capacity(self):
        control = AsyncMock()
        history = CollectionHistory(control)
        self.assertEqual(await history.readiness([]), {})
        with self.assertRaises(ValueError):
            await history.readiness([uuid4() for _ in range(101)])
        control.run.assert_not_awaited()
        async with history.slot:
            with self.assertRaises(HistoryUnavailable):
                await history.readiness([uuid4()])
        control.run.assert_not_awaited()

    async def test_lineage_cursor_is_bound_to_observation_before_io(self):
        import base64
        from datetime import UTC, datetime
        ...
        history = CollectionHistory(control)
        identity = uuid4()
        anchor = LineageCursor(observation_id=identity,
        ...
        for cursor in ("!", "x" * 513,
                       encode(anchor.model_copy(update={"observation_id": uuid4()}))):
            with self.assertRaises(ValueError):
                await history.observation_lineage(identity,  limit=20, cursor=cursor)
        with self.assertRaises(ValueError):
