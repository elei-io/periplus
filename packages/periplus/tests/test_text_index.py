...
from periplus.query.optimizations.element_text import match, run, TextMatch
...
class TextIndexTests(unittest.TestCase):
    def setUp(self):
        ...
        for name in ("html_elements", "html_terms"):
            ...
            self.db.execute(
                f"INSERT INTO material.{name} SELECT * FROM projection_rows"
            )
            self.db.unregister("projection_rows")

    def rewrite(self, sql):
        return run(
            PassContext(_one_statement(sql), [], "experimental", "memory", self.db)
        )
    ...
    def test_exact_results_multiplicity_and_types_are_preserved(self):
        for sql in (
            ...
            with self.subTest(sql=sql):
                ...
                self.assertEqual(rewritten.status, "applied")
                self.assertEqual(
                    self.db.execute(
                ...
                self.assertIn("text", rewritten.statement.sql(dialect="duckdb").lower())
        self.assertGreater(
            len(
                self.db.execute(
                    "SELECT * FROM html_element WHERE text='The Requiem Red'"
                ).fetchall()
            ),
            3,
        )
        ...
        for sql in queries:
            with self.subTest(sql=sql):
                self.assertNotIsInstance(
                    match(
                        PassContext(_one_statement(sql), [], "experimental", "memory")
                    ),
                    TextMatch,
                )
        # This span exists, but page-once tokenization deliberately has no fish posting.
        self.assertTrue(
            self.db.execute("SELECT * FROM html_element WHERE text='fish'").fetchall()
        )
        self.assertEqual(
            self.db.execute(
                "SELECT * FROM material.html_terms WHERE term='fish'"
            ).fetchall(),
            [],
        )
    ...
    def test_candidate_keys_remain_a_required_scan_filter(self):
        rewrite = self.rewrite(
            "SELECT content_id FROM html_element WHERE text='The Requiem Red'"
        )
        self.assertGreaterEqual(rewrite.counts["candidate_contents"], 5)
        plan = self.db.execute(
            "EXPLAIN " + rewrite.statement.sql(dialect="duckdb")
        ).fetchone()[1]
        self.assertNotIn("HASH_JOIN", plan)
        self.assertIn("Filters:", plan)
        self.assertIn("NULL", rewrite.statement.sql(dialect="duckdb"))
        self.assertIn("rowid", rewrite.statement.sql(dialect="duckdb"))
        with patch("periplus.query.optimizations.element_text.MAX_CANDIDATE_NODES", 1):
            self.assertEqual(
                self.rewrite(
                    "SELECT * FROM html_element WHERE text='The Requiem Red'"
                ).status,
        ...
        with patch(
            "periplus.query.optimizations.element_text.MAX_NODES_PER_CONTENT", 1
        ):
            self.assertEqual(
                self.rewrite(
                    "SELECT * FROM html_element WHERE text='The Requiem Red'"
    ...
    def test_overflow_declines_without_losing_matches(self):
        ...
        with patch(
            "periplus.query.optimizations.element_text.MAX_CANDIDATE_CONTENTS", 1
        ):
            self.assertEqual(self.rewrite(sql).status, "budget_exceeded")

    def test_changed_catalogue_and_collation_decline(self):
        sql = "SELECT * FROM html_element WHERE text='The Requiem Red'"
        self.db.execute("SET default_collation='nocase'")
        self.assertEqual(self.rewrite(sql).status, "contract_mismatch")
        self.db.execute("SET default_collation=''")
        self.db.execute(
            "CREATE OR REPLACE VIEW experimental.html_element AS SELECT content_sha256 AS content_id,node_index,text FROM material.html_elements WHERE false"
        )
        self.assertEqual(self.rewrite(sql).status, "contract_mismatch")
