...
class QueryServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.app.state.query_limits.read.return_value = ExecutionContext(limits=QueryLimits(), publication=PublicationBinding(expires_at="2099-01-01T00:00:00Z", database="public_v1", revision=0))
    ...
    def test_auth_routes_and_installed_capabilities(self):
        ...
        self.assertEqual({item['name'] for item in helpers['relations']},
            {'public_v1.capture','public_v1.page','public_v1.html_element','public_v1.link','public_v1.html_jsonld','public_v1.html_metadata'})
    ...
    def test_operator_row_limits_are_read_for_each_operation(self):
        ...
        for count in (2,7):
            self.app.state.query_limits.read.return_value = ExecutionContext(limits=QueryLimits(max_rows=count), publication=PublicationBinding(expires_at="2099-01-01T00:00:00Z", database="public_v1", revision=0))
