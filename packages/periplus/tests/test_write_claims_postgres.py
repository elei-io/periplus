...
from periplus.retention.identities import _acquire, WriteClaimUnavailable, write_claims
...
class PostgresClaimTests(unittest.TestCase):
    def test_first_creation_race_has_one_owner_and_expiry_allows_recovery(self):
        ...
        try:
            ...
            with patch.object(session, 'SessionLocal', sessionmaker(bind=engine)), ThreadPoolExecutor(2) as pool:
                ...
                with engine.begin() as connection:
                    connection.execute(text("UPDATE write_claims SET expires_at = now() - interval '1 second'"))
                    self.assertEqual(connection.scalar(text('SHOW transaction_timeout')), '4min')
                with write_claims({'capture': ['a']}):
                    pass
                with engine.begin() as connection:
                    self.assertEqual(connection.scalar(text('SELECT count(*) FROM write_claims')), 0)
        finally:
            engine.dispose()
            with base.begin() as connection:
