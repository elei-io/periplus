...
import {
  useContentPolicy,
  useDeleteContentPolicy,
  useUpdateContentPolicy,
} from "@/hooks/use-resource-data"
import type { ContentPolicyDetailRecord } from "@/types/resources"
import {
  PolicyForm,
  policyDraft,
  policyDraftError,
...
export function ContentPolicyDetailPage({ policyId }: { policyId: string }) {
  const query = useContentPolicy(policyId)
  ...
  return (
    <div className="flex min-h-0 w-full flex-col gap-4">
      ...
      <div className="border-t pt-3 text-xs text-muted-foreground">
        Policy key <span className="font-mono">{policy.slug}</span> · Updated{" "}
...
function ExistingPolicyForm({ policy }: { policy: ContentPolicyDetailRecord }) {
  const update = useUpdateContentPolicy(policy.id)
  const remove = useDeleteContentPolicy(policy.id)
  ...
  const deletePolicy = () => {
    if (
      !window.confirm(
        "Delete this policy? A less-specific rule will take over for these pages."
      )
    )
      return
  ...
  return (
    <>
      <PolicyForm draft={draft} onChange={setDraft} defaultPolicy={isDefault} />
      <div
        className={`sticky bottom-3 z-10 flex items-center gap-3 ${isDefault ? "justify-end" : "justify-between"}`}
      >
        {!isDefault ? (
          <Button
            variant="destructive"
        ...
        <Button
          ...
          onClick={save}
        >
          <SaveIcon /> {update.isPending ? "Saving…" : "Save policy"}
        </Button>
      </div>
    </>
  )
...
function coverageLabel(policy: ContentPolicyDetailRecord) {
  ...
  return policy.path_mode === "exact"
    ? `${host}${policy.path_prefix} only`
    : `${host}${policy.path_prefix} and below`
}
