...
export interface CrawlerActivity {
  ...
  waiting_reasons: Array<
    "ingestion_delivery_unavailable" | "storage_unavailable" | "cdp_unavailable"
  >
...
export interface LiveView {
  ...
  current: {
    ...
    domains: Array<{
      domain: string
      unique_queued_urls: number
      request_queued_urls: number | null
      queued: number
      dispatched: number
      started: number
      oldest_wait_at: string | null
  ...
  history: {
    ...
    velocities: Array<{
      seconds: 60 | 300
      domain: string | null
      attempt_starts: number
      successful_captures: number
      failed_captures: number
      fulfillments: number
      attempt_starts_per_minute: number
    }>
    recent: RecentCapture[]
  } | null
