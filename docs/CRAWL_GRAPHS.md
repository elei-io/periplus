...
4. Atomically create the request, admission marker, counters, and a `CrawlWork` outbox row.
...
8. Acquisition atomically checkpoints readiness into the outbox. Each outgoing edge runs against
   the current page package and
...
- `GraphRun`, `CrawlRequest`, and `EdgeEvaluation`: current Postgres-owned execution state.
- `GraphOutbox`: transactionally coupled work intent, published with a deterministic NATS message ID.
...
ACK requires retained HTML, durable ingestion publication, and a committed navigation-readiness
outbox row. A run cancellation fences the current generation and settles every nonterminal request.
Pause suspends outbox publication and causes delivered work to be NAKed without changing its
checkpoint; resume makes the same frontier eligible again. Pause does not extend the persisted
wall-clock deadline. Ingestion terminal state and
...
Every run persists a wall-clock deadline, defaulted from `ATLAS_GRAPH_MAX_RUN_SECONDS` (seven days)
and optionally extended per run up to one year. Expiry stops admission and settles every
nonterminal request. Execution state is operational rather than historical; terminal Postgres rows
are retained for `ATLAS_GRAPH_RUN_RETENTION_SECONDS` (thirty days by default) and then deleted in
bounded housekeeping batches. Navigation objects are retained for the
terminal grace period, independently of the run's configured duration. Durable crawl and DOM
evidence remains in DuckLake.
Terminal request settlement also updates a bounded per-run failure summary in Postgres, grouped by
...
Postgres row locks cover only bounded state transitions; policy and definition reads are batched,
and no database session is held across CDP, object-store, NATS, or DuckBasin work. The engine uses a
fixed-size, no-overflow connection pool so worker concurrency cannot silently multiply database
connections.
