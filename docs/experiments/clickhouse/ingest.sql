-- Proposed schema, not an application migration or a startup script.
-- Run against an empty scratch database selected with clickhouse-client --database.
-- Logical uniqueness, frozen-evidence validation, and replay are writer responsibilities.
-- Collections and terminal request accounting remain in control Postgres.
-- DESIGN_REVIEW.md lists acquisition evidence additions still required before freezing this DDL.
...
CREATE TABLE acquisition_reasons
(
    record_id UUID,
    recorded_at DateTime64(6, 'UTC'),
    observation_id UUID,
