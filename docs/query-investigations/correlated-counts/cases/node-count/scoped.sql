WITH keys AS MATERIALIZED (SELECT DISTINCT content_id FROM public_v1.capture WHERE effective_url LIKE '%.gov%' AND content_id IS NOT NULL ORDER BY content_id LIMIT $scope), counts AS MATERIALIZED (
