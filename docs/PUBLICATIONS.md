...
4. a public wrapper view that keeps the original view name stable.
...
DML ticks intentionally contain no row keys. A materialization therefore
coalesces ticks for its configured delay and performs one whole-table refresh:

```sql
BEGIN;
DELETE FROM _atlas_materializations.target;
INSERT INTO _atlas_materializations.target
SELECT * FROM (<stored source SQL>);
COMMIT;
```
...
the incarnation. Downstream sinks may choose their own coalescing/latency
trade-off.
